...
/**
 * Rivers only, and named ones at that.
 *
...
async function main(): Promise<number> {
  ...
  for (const way of response.elements) {
    ...
    // Keep a river that crosses the box even if most of it is outside: the Ping runs
  ...
  const out: ValleyFeatures = {
    _comment:
      'Rivers and towns for the VALLEY view. Rivers from OpenStreetMap under ODbL; ' +
      'towns from GeoNames under CC BY 4.0. Both credited in the viewer footer. ' +
      'Generated by scripts/fetch-valley.ts — do not hand-edit.',
    halfKm: HALF_KM,
    rivers,
    towns,
  };
