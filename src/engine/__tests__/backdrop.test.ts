...
import { screenFootprint } from '../camera';
import { SURFACE_ROLES } from '../theme';

describe('projectView, as the backdrop uses it', () => {
   ...
   * `screenFootprint`, and the generator sizes the raster from this projection. If the
   * two disagreed, the backdrop would be the right picture at the wrong scale — which
   * reads as a seam, not as an error. They are the same function now; this is the
   * test that says so.
   */
  it('agrees with isometricFit about the screen footprint of a ground rectangle', () => {
    ...
    const xs = corners.map(([x, y]) => projectView(x, y)[0]);
    const ys = corners.map(([x, y]) => projectView(x, y)[1]);

    const width = Math.max(...xs) - Math.min(...xs);
    const height = Math.max(...ys) - Math.min(...ys);
    const footprint = screenFootprint([0, 0, spanX, spanY]);

    expect(width).toBeCloseTo(footprint.width, 6);
    expect(height).toBeCloseTo(footprint.height, 6);
  });
...
describe('planeRect', () => {
  ...
  it('maps a world point to the same screen point the geometry would', () => {
    ...
    const [sx, sy] = projectView(120, 80, 6);
